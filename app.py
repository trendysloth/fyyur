    ...
    for v in venues_:
      ...
      for show in shows:
        if (show.venue_id in showId):
          ...
          current_show = {
            "id": show.venue_id,
            "name": Venue.query.filter_by(id=show.venue_id).first().name,
            "num_upcoming_shows": num_upcoming_shows
          }
          venues.append(current_show)
      cityStatePair = v.city + ', ' + v.state
      if (not cityStatePair in cityState):
        data.append({
          "city": v.city,
          "state": v.state,
          "venues": venues
...
def create_artist_submission():
  ...
  try:
    ...
    flash(f'An error occurred. Artist ' + data.name + ' could not be listed. Error: {e}')
    db.session.rollback()
    return render_template('forms/new_artist.html', form=form)
  finally:
    db.session.close()
